...
const Applications: React.FC<{ userData?: { role: string } }> = ({ userData }) => {
  ...
  const filteredApplications = useMemo(() => {
    ...
    if (role === "hr") apps = apps.filter((a) => a.status !== "rejected");
    if (selectedJob !== "all") apps = apps.filter((a) => a.job_title === selectedJob);
    return apps;
      ...
      <div className="flex items-center justify-between mt-4">
        ...
        <button
          ...
          className={`px-3 py-1 rounded border ${
            page === totalPages || totalPages === 0 ? "opacity-50 cursor-not-allowed" : "hover:bg-gray-100"
          }`}
        >
          Next
        </button>
