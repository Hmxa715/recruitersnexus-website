...
const Applications: React.FC<{ userData?: { role: string } }> = ({ userData }) => {
  ...
  const filteredApplications = useMemo(() => {
    ...
    // HR sees only pending and shortlisted
    if (role === "hr") {
            ...
            {loading ? (
              ...
              paginatedApplications.map((app) => (
                <tr key={app.id} className="hover:bg-gray-50">
                  ...
                  <td className="border p-2">{app.applied_at ? new Date(app.applied_at).toLocaleString() : "—"}</td>
                  <td className="border p-2 capitalize">{app.status}</td>
                  <td className="border p-2 space-x-2">
                    {(role === "hr" || role === "admin") && app.status === "pending" && (
                      <>
                        <button
      ...
      {selectedApp && details && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-[800px] max-h-[90vh] overflow-y-auto shadow-lg">
            <h2 className="text-xl font-bold mb-4">{selectedApp.username} - {selectedApp.job_title}</h2>
            ...
            <div className="mt-6 text-right">
              <button
                onClick={() => { setSelectedApp(null); setDetails(null); }}
