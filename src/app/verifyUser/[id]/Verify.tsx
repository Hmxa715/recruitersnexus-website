...
const Verify = ({ id }: any) => {
  ...
  const filteredVerify: verifyData[] = verification.filter(
    (item: any) => item.user_id === id
  );
    ...
    try {
      ...
      if (res.ok) {
        ...
        try {
          ...
          router.push(
            `/verifyUser/${id}?hashedRegistrationCode=${hashedRegistrationCode}`
          );
          return res;
        } catch (error) {
          //console.log(error);
  ...
  async function onSubmit() {
    ...
    // const salt = await bcryptjs.genSalt(10);
    // const hashedCode = await bcryptjs.hash(securityCode, salt);
    // alert("Hashed Security from Table: " + hashedRegistrationCode);
    // alert("Hashed Security from Input: " + securityCode);

    // alert("Hashed Security from Table: "+ hashedRegistrationCode + "\nHashed Security from Input: " + hashedSecurity);
    if (hashedRegistrationCode !== null) {
      ...
      if (validPassword) {
        ...
        const otpReg = Math.floor(100000 + Math.random() * 900000);
        // const hashedRegistrationCode = await bcryptjs.hash(otpReg.toString(), salt);
        // alert("Valid pass: " + validPassword);
        toast.success("Thank you for submitting the security code!");
        try {
          const res = await fetch("/api/verify", {
            method: "PUT",
            body: JSON.stringify({
          ...
          if (res.ok) {
            // alert("User is successfully Verified!")
